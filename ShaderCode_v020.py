...
def ImportLatestShaders():
    ...
    for i in range(len(selected)):
        ...
        if(os.path.exists(LatestShaderMB_Dir) & os.path.exists(LatestShaderJSON_Dir)):
            ...
            mc.file(LatestShaderMB_Dir, i=True, type="mayaBinary", ra=True, rdn=True, mnc=True, an=True)
            ...
            for set in obj:
                Mesh_Material=[]
                for i in set.split(':'):            
                    Mesh_Material.append(i)
                print("#1: " + Mesh_Material[0] + " #2: " + Mesh_Material[1])
                Mesh = mc.ls("*:"+Mesh_Material[0]+"*")[0]
                Material = mc.ls("*"+Mesh_Material[1]+"*")[0]
                
                mc.select(Mesh)
                
                #MeshShadingEngine = mc.listConnections(Mesh, type="shadingEngine")[0]
                #MeshSurfaceShader = mc.listConnections(selectedMeshShading + ".surfaceShader")[0]
                print("Mesh: " + Mesh)
                print("Material: " + Material)
                
                DefaultNav = mc.defaultNavigation(ce=True, s=Material, d=Material+"SG")
                mc.connectAttr(Material+".outColor", Material+"SG.surfaceShader",f=True)
                mc.sets(e=True, fe=Material+"SG")
                
                #mc.connectAttr(Material+".surfaceShader", f=True)[0]
